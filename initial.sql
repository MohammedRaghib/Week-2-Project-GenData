...
SELECT sd.stud_ID, sd.stud_email, pd.stud_name, pd.phone_number
FROM school_details sd
LEFT JOIN personal_details pd ON sd.stud_ID = pd.stud_ID;

-- 2.
CREATE VIEW IF NOT EXISTS full_stud_details AS
SELECT sd.stud_ID,
	pd.national_ID,
	pd.stud_name,
	sd.stud_email,
...
FROM school_details sd
LEFT JOIN personal_details pd ON sd.stud_ID = pd.stud_ID
LEFT JOIN contact_details cd ON sd.stud_email = cd.stud_email
LEFT JOIN financial_details fd ON sd.stud_ID = fd.stud_ID
ORDER BY pd.stud_name ASC;
...
SET fee_cleared = CASE WHEN fee_paid >= sem_fee THEN TRUE ELSE FALSE END;
